...
class MultiProtoAsConv(nn.Module):
    ...
    def forward(self, mol, qry, sup_x, sup_y, s_init_seed, mode, thresh, isval = False, val_wsize = None, vis_sim = False, **kwargs):
        ...
        sup_y = sup_y.squeeze(0) # [nshot, rater, h, w]

        # Build per-rater prototypes by iterating along rater axis of sup_y
        # sup_y: [nshot, rater, h, w]
        rater_dim = sup_y.shape[1] if sup_y.dim() > 1 else 1
        per_rater_protos = []
        per_rater_counts = []
        for r in range(rater_dim):
            sup_y_r = sup_y[:, r:r+1, :, :]
            proto_r = self.get_proto(mode, mol, qry, sup_x, sup_y_r, s_init_seed, thresh, isval, val_wsize)  # [n_proto, C]
            # record how many prototypes this rater produced
            n_proto = proto_r.shape[0] if (proto_r is not None and proto_r.dim() > 1) else 1
            per_rater_counts.append(n_proto)
            per_rater_protos.append(proto_r)

        # concatenate into [total_proto, C]
        per_rater_protos = torch.cat(per_rater_protos, dim=0)  # [total_proto, C]
        # compute start indices for splitting later
        per_rater_starts = [0]
        for c in per_rater_counts[:-1]:
            per_rater_starts.append(per_rater_starts[-1] + c)

        consensus_mask = sup_y.mean(dim=1, keepdim=True)  # [1, 1, h, w]

        # consensus prototype p0 (keep as forward's proto to preserve behavior)
        ...
        # no need to merge with gridconv+
        ...
        elif mode == 'mask++': # class-level prototype only (per-rater calibrated)
            qry_n = self.safe_norm(qry)
            per_rater_preds = []
            per_rater_assigns = []
            raw_sims_list = []
            for cal in per_rater_calibrated:
                if cal.numel() == 0:
                    ...
                    continue
                pro_n = self.safe_norm(cal)
                # reduce to a single representative prototype for class-level scoring
                if pro_n.dim() > 1:
                    proto_mean = pro_n.mean(dim=0, keepdim=True)
                else:
                    proto_mean = pro_n
                dists = F.conv2d(qry_n, proto_mean[..., None, None]) * 20
                pred = torch.sum(F.softmax(dists, dim=1) * dists, dim=1, keepdim=True)
                assign = dists.argmax(dim=1).float().detach()
                per_rater_preds.append(pred)
                per_rater_assigns.append(assign)
        ...
        elif mode == 'bg': # using local prototypes only (per-rater calibrated)
            qry_n = self.safe_norm(qry)
            per_rater_preds = []
            per_rater_assigns = []
            raw_sims_list = []
    ...
    def get_proto(self, mode, mol, qry, sup_x, sup_y, s_init_seed, thresh, isval=False, val_wsize=None):
        """Extract prototypes / proto for different modes. Returns:
           - for 'mask' and 'mask++': proto (nb x C or 1 x C)
           - for 'gridconv' and 'bg': protos (npro x nc)
           - for 'gridconv+': (protos, proto)
        """
        if mode == 'mask':
            ...
            if (num_sp == 0):
                ...
                if mol == 'alignLoss':
                    ...
                    for sup_n in range(sup_nshot):
                        with torch.no_grad():
                            s_seed__ = s_seed_[:num_sp, :]
                            sp_init_center = sup_x[sup_n][:, s_seed__[:, 0], s_seed__[:, 1]]
                            sp_init_center = torch.cat([sp_init_center, s_seed__.transpose(1, 0).float()], dim=0)
                            sp_center = self.sp_center_iter(sup_x[sup_n], sup_y[sup_n], sp_init_center, n_iter=10)
            ...
            w1 = torch.mm(n_sup_x.float(), n_sup_x.permute(1, 0).float())
            ...
            # concatenate local protos and global proto then normalize
            all_protos = torch.cat([protos, proto], dim=0)
            pro_n = self.safe_norm(all_protos)
            return pro_n
