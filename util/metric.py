...
class Metric(object):
    ...
    def record(self, pred, target, labels=None, n_scan=None):
        ...
        tp_arr = np.full(len(self.labels), np.nan) # array([nan, nan, nan, nan, nan])
        fp_arr = np.full(len(self.labels), np.nan) # array([nan, nan, nan, nan, nan])
        fn_arr = np.full(len(self.labels), np.nan) # array([nan, nan, nan, nan, nan])
        ...
        for j, label in enumerate(labels):
            ...
            tp_arr[label] = len(set.intersection(pred_idx_j, target_idx_j)) # 交集
